...
export class Controller extends BaseController implements IController {
  public name = 'Sociol link';
  constructor(
    @inject(TYPES.ILogger) private loggerService: ILogger,
    @inject(Social.TYPES.Service) private service: IService,
  ...
  public find = async (req: Request<GetDto, unknown, unknown>, res: Response, next: NextFunction): Promise<void> => {
    const key = await this.cacher.get<Social.Entity>(req.params.id);
    if (key) {
    ...
    await this.cacher.set<Social.Entity>(req.params.id, result);
